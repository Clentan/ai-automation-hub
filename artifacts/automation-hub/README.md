...
All data comes from the FastAPI backend under the `/api` prefix (see [`../api-server`](../api-server)). Requests are sent with the app's base URL prefix so they work in every environment. Sign-in uses Clerk; the browser session cookie is verified by the backend.
