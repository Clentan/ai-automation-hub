...
The server connects to PostgreSQL with a psycopg connection pool and creates its own tables on startup (`CREATE TABLE IF NOT EXISTS` in `main.py`): `templates`, `user_api_keys`, `runs`, `template_requests`, `user_settings`, and `digest_log`. See [`../../lib/db`](../../lib/db) for how this relates to the shared database package.
