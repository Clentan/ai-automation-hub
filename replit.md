...
- Backend: FastAPI (Python) in artifacts/api-server, served under /api (SQLite at artifacts/api-server/data/hub.db, gitignored). Handles per-template API keys (issue/regenerate/revoke, per-browser X-Client-Id identity), template requests, and a LIVE run endpoint POST /api/v1/templates/{id}/run that validates the Bearer key belongs to that template. Frontend calls it via src/lib/use-api-keys.ts. Flows/activity/settings remain localStorage by design (no auth yet).
